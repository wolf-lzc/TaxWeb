...
<body>
	<div class="main">
		<div class="tab_bt">
			<p class="bt">
				<span>利润表</span><a href="${ctx}/profit/all/download?type=${type}" class="down">下载</a>
			</p>
			<p class="nr">纳税人名称：${sessionScope.company}</p>
			<p class="nr">纳税人识别号：${sessionScope.tax_code}</p>
		</div>
		<ul class="tab_xxk">
			<li id="li_1"><a href="${ctx }/profit/list" target="_self">近3个月</a></li>
			<li id="li_2"><a href="${ctx }/profit/list?year=${nowYear-1}"
				target="_self">${nowYear-1}年</a></li>
			<li id="li_3"><a href="${ctx }/profit/list?year=${nowYear-2}"
		...
		<div class="tab_item">
				...
				<c:if test="${fn:length(nowYearData) gt 0}">
					<ul class="zc_list">
						...
						<c:forEach var="data" items="${nowYearData}">
							<li class="nr">
								<a href="${ctx}/profit/info?date=${data.reportDate}">
									<p>
			...
			<c:if test="${type eq 2}">
				<ul class="zc_list">
					<li class="bt"><span>${nowYear-1}年</span></li>
					<c:forEach var="data" items="${lastYearData}">
						<li class="nr"><a href="${ctx}/profit/info?date=${data.reportDate}">
								<p>
									报表日期： <span>${fn:substring(data.reportDate,0,4)}年${fn:substring(data.reportDate,4,6)}月</span>
								</p> <img src="${ctx}/images/right_tb.png" />
								<div class="clear"></div>
